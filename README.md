...
- Map passed options to an object containing all the relevant "context" (be it objects passed through Preact context providers, or other useful values for controlling the test harness)
- Use the resolved context to render Preact components around the element under test that use the context
...
### `.toContainComponent(type: string | Preact.ComponentType, props?: object)`
...
### `.toContainComponentTimes(type: string | Preact.ComponentType, times: number, props?: object)`
...
### `.toProvideContext<T>(context: Context<T>, value?: T)`
