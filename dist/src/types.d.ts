...
export declare type ComponentType<P extends {}> = OriginalComponentType<P>;
export declare type PropsFor<T extends string | ComponentType<any>> = T extends string ? T extends keyof JSX.IntrinsicElements ? JSX.IntrinsicElements[T] : JSX.HTMLAttributes : T extends ComponentType<any> ? PropsWithoutRef<T> : never;
export declare type PropsWithoutRef<P> = Pick<P, Exclude<keyof P, 'ref'>>;
...
export declare type Predicate = (node: Node<any>) => boolean;
export interface Node<Props> {
    ...
    text(): string;
    html(): string;
    is<Type extends ComponentType<any> | string>(type: Type): this is Node<PropsFor<Type>>;
    find<Type extends ComponentType<any> | string>(type: Type, props?: Partial<PropsFor<Type>>): Node<PropsFor<Type>> | null;
    findAll<Type extends ComponentType<any> | string>(type: Type, props?: Partial<PropsFor<Type>>): Node<PropsFor<Type>>[];
    findWhere(predicate: Predicate): Node<any> | null;
    findAllWhere(predicate: Predicate): Node<any>[];
    trigger<K extends FunctionKeys<Props>>(prop: K, ...args: DeepPartialArguments<Arguments<Props[K]>>): Promise<MaybeFunctionReturnType<NonNullable<Props[K]>>>;
...
export declare type NodeTree = (Node<any> | string)[];
export {};
